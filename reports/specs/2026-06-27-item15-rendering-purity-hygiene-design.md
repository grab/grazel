...
Close out the altitude-layering pass: confirm renderers are pure, kill dead/speculative
code, and add the one missing test. Pure cleanup — golden empty-diff.
...
2. **Delete dead-code residues** (from the earlier audit):
   ...
   - the dead `materializedMavenRepos` fallback method in `MavenInstallArtifactsCalculator`
     that is unreachable in the live path (render plan supplies the repo set).
...
4. **Confirm renderer/workspace purity** (verify, document): `ProjectBazelFileBuilder`,
   `RootBazelFileBuilder`, `WorkspaceBuilder`, pinner do no ownership inference, no
   generated-output reads, no reachability, no iterate-to-convergence. Any residual
   generated-output feedback (none expected after Items 3–4) is removed.
...
- Renderer purity confirmed/documented; zero generated-output feedback.
