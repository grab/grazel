...
class WorkspacePlanTasksTest {
    ...
    /**
     * Guards the transitive closure two hops deep: `c` (a test-only consumer) references `util1`,
     * which in turn is only referenced (via its own testImplementation) from `util2`. Both hops must
     * be discovered within the single consumers-first pass so `util2` - reachable only through a
     * chain of references, never directly from `c` - lands in the render plan. This is the scenario
     * the referenced-but-unreached Android library fallback (in `AndroidLibraryTargetBuilder` and
     * `TargetReferenceFactsExtractor`) depends on: if either twin `reachableMatchedVariants` site
     * regressed and stopped consulting the render plan, `util2` would never surface here either.
     */
    @Test
    fun `collect target references reaches transitive test-only dependency two hops deep`() {
        val rootProject = buildProject("root")
        ...
        val references: TargetReferenceFacts = collectTargetMavenRepoReferencesByGroup(
            projectGroups = listOf(cProject, util1Project, util2Project).map { project ->
                ProjectReachabilityGroup(listOf(project))
            },
            canMigrate = { true },
            factsForProject = { project ->
                when (project.path) {
                    ...
                    ":util1" -> if (
                        workspaceRenderPlanService.isReferencedTarget(":util1", "util1-gps-pax-debug")
                    ) {
                        TargetReferenceFactsCollector.from(
        ...
        assertEquals(setOf(":util1", ":util2"), references.projectPaths)
        assertEquals(
            mapOf(
                ":util1" to setOf("util1-gps-pax-debug"),
                ":util2" to setOf("util2-gps-pax-debug")
            ),
            references.projectTargets
        )
        assertTrue(workspaceRenderPlanService.isReferencedTarget(":util2", "util2-gps-pax-debug"))
        assertEquals(
            setOf("util2-gps-pax-debug"),
            workspaceRenderPlanService.referencedTargetNames(":util2")
        )
    }
}
